...
from .models import Coordinate, Category, FollowUp
from .forms import (

    CoordinateModelForm, 
    CoordinateCategoryUpdateForm,
    CategoryModelForm,
    FollowUpModelForm
)
...
class CoordinateListView(LoginRequiredMixin, generic.ListView):
    template_name = "coordinates/coordinate_list.html"
    ...
    def get_queryset(self):
        ...
        if user.is_trainee:
            queryset = Coordinate.objects.filter(
                profile=user.filter(is_coordinator = True) 
        )
        else:
...
class CoordinateDetailView(LoginRequiredMixin, generic.DetailView):
    template_name = "coordinates/coordinate_detail.html"
    context_object_name = "coordinate"

    def get_queryset(self):t6
        user = self.request.user
        # initial queryset of leads for the entire organisation
